    ...
    <div className="min-h-screen bg-[#0f172a] text-slate-200 p-8">
      ...
      <header className="flex justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black text-white">CASHIER</h1>
      ...
      {activeZone?.status === "CLOSED" && (
        <div
          className="mb-8 p-4 rounded-xl bg-rose-900/40
        ...
        {zoneTables.map((t) => {
          ...
          return (
            <div key={t.id} className="relative">
              <button
                disabled={zoneClosed}
                onClick={() => !zoneClosed && setSelectedTable(t)}
              ...
              >
                <div className="text-2xl font-black">โต๊ะ {t.number}</div>

                <div className="text-sm flex items-center gap-2">
                  <Clock size={16} />
                  {isOpen ? formatTime(timer?.remainingSeconds) : "AVAILABLE"}
                </div>
              </button>
            </div>
          );
